  ...
  <div ref="rootRef" class="relative" @click.stop>
    <div class="cursor-pointer flex gap-2 items-center justify-between -mx-4 -my-2 px-4 py-2" @click="toggle">
      <span class="flex gap-2 items-center">
        {{ fileFormat === 'xlsx' ? $t('Export to Excel') : $t('Export to CSV') }}
...
async function exportFile(select: string) {
  inProgress.value = true;
  try {
    const resp = await callAdminForthApi({
    ...
    const filenameBase = `export-${coreStore.resource?.resourceId}-${new Date().toISOString()}`;
    if (fileFormat === 'xlsx') {
      const XLSX = await import('xlsx');
      const worksheet = XLSX.utils.aoa_to_sheet([resp.data.fields, ...resp.data.data]);
      ...
      const csvContent = '﻿' + Papa.unparse(resp.data, {
        quotes: resp.columnsToForceQuote,
        quoteChar: '"',
        escapeChar: '"',
      });
